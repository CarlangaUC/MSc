...
def compilar_dataset(txt_path, boundaries_path, log_path, output_docs, output_mapping, output_mapping_txt, output_txt):
    # ==========================================
    ...
    # --- NUEVO: Guardar mapeo en archivo de texto plano para lectura humana ---
    with open(output_mapping_txt, 'w', encoding='utf-8') as f_map_txt:
        f_map_txt.write("MAPEO DE DOCUMENTOS MAESTROS A VERSIONES GLOBALES\n")
        f_map_txt.write("="*55 + "\n")
        for i in range(len(page_starts) - 1):
            inicio = page_starts[i]
            fin = page_starts[i+1] - 1
            cantidad = page_starts[i+1] - page_starts[i]
            f_map_txt.write(f"Documento Maestro {i} | Rango Global: [{inicio} - {fin}] | Total Versiones: {cantidad}\n")
    print(f"   -> Mapeo legible guardado en {output_mapping_txt}.")
